...
class MyRequest:
    ...
    # 從yfinanace取得資料
    def GetStockInfoV3(self):
        print(self.reqParam)
        stock = yf.Ticker(f"{self.reqParam['stockCode']}.TW")
        df = stock.history(start=self.reqParam['fromDate'], end=self.reqParam['toDate'], interval=self.reqParam['time'])
        df['Date'] = df.index.map(lambda x:x.strftime('%Y-%m-%d'))     
        collect = df.loc[:,["Date","Close"]]
        js = collect.to_json(orient = 'records')
        result = json.loads(js)
